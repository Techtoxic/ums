...
const studentNotes = pgTable('student_notes', {
    ...
    // V1 note metadata (added in migration 0010). note_type drives the
    // public-notes route; the rest are denormalised display fields.
...
const studentUploads = pgTable('student_uploads', {
    ...
    // V1 upload metadata (added in migration 0010): upload typing, unit linkage,
    // S3 keys, versioning + replacement chain, academic period, denormalised
    // student fields. category remains the NOT NULL classifier (= upload_type).
    upload_type: text('upload_type'),
